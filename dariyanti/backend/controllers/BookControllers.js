...
// Helper untuk format tanggal (versi backend - Node.js)
const formatDateForMidtrans = (dateString) => {
  return new Date(dateString).toLocaleDateString("id-ID", {
    day: 'numeric',
    month: 'short', // 'short' agar lebih ringkas, misal: '26 Okt 2025'
    year: 'numeric'
  });
};

// Helper untuk memetakan kode tipe kamar ke nama lengkapnya
// (Diambil dari BookingDetail.jsx Anda)
...
export const createMidtransTransaction = async (req, res) => {
  try {
    ...
    // Siapkan detail untuk nama item
    const roomName = getRoomTypeName(booking.roomType);
    const checkInFormatted = formatDateForMidtrans(booking.checkIn);
    const checkOutFormatted = formatDateForMidtrans(booking.checkOut);

    const checkInParts = checkInFormatted.split(' ');   // Hasil: ["31", "Okt", "2025"]
    const checkOutParts = checkOutFormatted.split(' '); // Hasil: ["1", "Nov", "2025"]

    const checkInDay = checkInParts[0];
    const checkInMonth = checkInParts[1];
    const checkInYear = checkInParts[2];

    const checkOutDay = checkOutParts[0];
    const checkOutMonth = checkOutParts[1];
    const checkOutYear = checkOutParts[2];

    let dateString = "";

    if (checkInMonth === checkOutMonth && checkInYear === checkOutYear) {
        // Kasus 1: Bulan & Tahun SAMA (e.g., "26-27 Okt 2025")
        dateString = `${checkInDay}-${checkOutDay} ${checkInMonth} ${checkInYear}`;
    
    } else if (checkInYear === checkOutYear) {
        // Kasus 2: Bulan BEDA, Tahun SAMA (e.g., "31 Okt - 1 Nov 2025")
        // Ini adalah format yang Anda minta
        dateString = `${checkInDay} ${checkInMonth} - ${checkOutDay} ${checkOutMonth} ${checkInYear}`;
    
    } else {
        // Kasus 3: Tahun BEDA (e.g., "31 Des 2025 - 1 Jan 2026")
        dateString = `${checkInFormatted} - ${checkOutFormatted}`;
    }

    // Gabungkan dengan nama kamar
    const itemName = `${roomName}, ${dateString}`;
    ...
    const parameter = {
      ...
      item_details: [
        {
          id: booking.bookingCode,        // ID Booking Anda
          price: booking.total,         // Harga total
          quantity: 1,                    // 1 paket booking
          name: itemName,                 // Nama item yang deskriptif
          brand: 'Fhandika Boutique Hotel', // (Opsional)
          category: 'Akomodasi Hotel',      // (Opsional)
        }
      ],
      customer_details: {
...
export const regenerateToken = async (req, res) => {
  ...
  try {
    ...
    // Siapkan detail untuk nama item (SAMA SEPERTI FUNGSI SEBELUMNYA)
    const roomName = getRoomTypeName(existingBooking.roomType);
    const checkInFormatted = formatDateForMidtrans(existingBooking.checkIn);
    const checkOutFormatted = formatDateForMidtrans(existingBooking.checkOut); // Tidak perlu

    const checkInParts = checkInFormatted.split(' ');
    const checkOutParts = checkOutFormatted.split(' ');
    ...
    if (checkInMonth === checkOutMonth && checkInYear === checkOutYear) {
        // Kasus 1: Bulan & Tahun SAMA
        dateString = `${checkInDay}-${checkOutDay} ${checkInMonth} ${checkInYear}`;
    
    } else if (checkInYear === checkOutYear) {
        // Kasus 2: Bulan BEDA, Tahun SAMA
        dateString = `${checkInDay} ${checkInMonth} - ${checkOutDay} ${checkOutMonth} ${checkInYear}`;

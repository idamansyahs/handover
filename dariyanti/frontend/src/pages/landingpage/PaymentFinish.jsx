...
const PaymentFinish = () => {
    ...
    const handleResumePayment = async() => {
        ...
        try {
            ...
            const errorMsg = error.response?.data?.error || ""; // e.g., "Status pesanan saat ini adalah: CANCELLED"
            
            if (errorMsg.includes("Status pesanan saat ini adalah: CONFIRMED")) {
                setMessage({ text: "Pesanan ini sudah dikonfirmasi. Memuat ulang status...", type: 'success' });
                setPaymentStatus({
                    status: 'success', // Ini memicu UI "Pembayaran Berhasil!"
                    message: 'Pembayaran Berhasil!',
                    orderId: bookingDetails.bookingCode
                });
            } else if (errorMsg.includes("Status pesanan saat ini adalah: CANCELLED") || errorMsg.includes("Status pesanan saat ini adalah: CONFIRMED")) {
                setMessage({ text: "Pesanan ini telah dibatalkan atau selesai dan tidak bisa dibayar.", type: 'error' });
                setPaymentStatus({
                    status: 'error',
                    message: 'Pesanan telah dibatalkan.', // Pesan utama juga diubah
                    orderId: bookingDetails.bookingCode
                });
            } else {
                ...
                {bookingDetails && (
                    <div className="booking-details-summary"> {/* Kelas ini masih dari PaymentFinish.css */}
                        ...
                        <ul className="list-group list-group-flush">
                            ...
                            <li className="list-group-item"><span>Nama Pemesan</span><strong>{bookingDetails.guestName}</strong></li>
                            <li className="list-group-item"><span>Phone</span><strong>{bookingDetails.phone}</strong></li>
                            <li className="list-group-item"><span>Email</span><strong>{bookingDetails.email}</strong></li>
                            ...
                            <li className="list-group-item"><span>Durasi</span><strong>{`${duration} malam (@${formatCurrency(bookingDetails.pricePerNight)}/malam)`}</strong></li>

...
export default function BookingForm() {
  ...
  // ==================== FUNGSI BARU UNTUK VALIDASI CHECK-OUT (DARI Bookings.jsx) ====================
  const getMinCheckOutDate = () => {
    // Jika checkIn belum dipilih, min checkout adalah 1 hari setelah hari ini
    const minDate = new Date(formData.checkIn || today);
  ...
  const handleChange = (e) => {
    ...
    // === LOGIKA VALIDASI CHECK-OUT BARU (DARI Bookings.jsx) ===
    // Jika user mengubah check-in
    if (name === "checkIn") {
      const newCheckInDate = new Date(value);
      
      // Cek jika checkOut sudah ada
      if (newFormData.checkOut) {
        const currentCheckOutDate = new Date(newFormData.checkOut);
        
        // Jika tanggal check-in baru SAMA DENGAN ATAU SETELAH check-out
        // (Perhitungan +1 hari karena min checkout adalah checkIn + 1)
        if (newCheckInDate >= currentCheckOutDate) {
           newFormData.checkOut = ""; // Reset tanggal check-out
        }
      }
    }
    // === AKHIR LOGIKA BARU ===
    ...
    // Reset status ketersediaan jika input kunci berubah
                  ...
                  <div className="row g-3"> {/* Use g-3 for gap */}
                    ...
                    <div className="col-md-6 mb-3">
                      <div className="input-group">
                         ...
                         <input type="date" className="form-control" name="checkOut" value={formData.checkOut} onChange={handleChange} required min={getMinCheckOutDate()} disabled={!formData.checkIn} />
                      </div>
                    </div>
                  </div>
                  <div className="input-group mb-3">
